...
- Dropped support for pyspades and pysnip.
...
def apply_system_patches():
    ...
    # Version changes
    #
    pyspades.constants.GAME_VERSION = 4
...
def apply_script(protocol, connection, config):
    ...
    class MalsaConnection(connection):
        def send_map(self, data = None, *args, **kwargs):
            if data is not None:
                self.map_data = data
                map_start = MalsaMapStart()
                map_start.size = data.get_size()
                # TODO: get CRC
                #map_start.crc = zlib.crc32(self.protocol.map_info.data) & 0xFFFFFFFF
                #map_start.map_name = self.protocol.map_info.rot_info.get_map_name()
    ...
    class MalsaProtocol(protocol):
        ...
        def send_contained(self, contained, *args, **kwargs):
            #print("Contained ID %d" % (contained.id,))
            if contained.id == pyspades.contained.WorldUpdate.id:
                #print("Wrap %s somehow" % (repr(contained),))
                world_update = MalsaWorldUpdate()
                world_update.items = contained.items
                contained = world_update
            else:
